package com.util;

import java.awt.Color;

public class Const {
	public static final int SCREEN_WIDTH = 1280;
	public static final int SCREEN_HEIGHT = 720;
	public static final String SCREEN_TITLE = "Geometry Dash";
	
	public static final int TILE_WIDTH = 42;
	public static final int TILE_HEIGHT = 42;
	public static final float PLAYER_SPEED = 395.0f;
	
	public static final int GROUND_Y = 672; // or 714
	public static final int CAMERA_OFFSET_X = 300;
	public static final int CAMERA_OFFSET_Y = 350;
	
	public static final int CAMERA_OFFSET_GROUND_Y = 100;
	public static final int GRAVITY = 2850;
	public static final int MAX_VELOCITY = 1900;
	
	public static final int BUTTON_OFFSET_X = 420;
	public static final int BUTTON_OFFSET_Y = 560;
	public static final int BUTTON_SPACING_HZ = 10;
	public static final int BUTTON_SPACING_VT = 5;
	
	public static final int BUTTON_WIDTH = 60;
	public static final int BUTTON_HEIGHT = 60;
	
	public static final double JUMP_FORCE = -650;
	
	public static final Color GB_COLOR = new Color(15.0f/255.0f, 98.0f/255.0f, 212.0f/255.0f, 1.0f);
	public static final Color GROUND_COLOR = new Color(28.0f/255.0f, 70.0f/255.0f, 148.0f/255.0f, 1.0f);
	
}
